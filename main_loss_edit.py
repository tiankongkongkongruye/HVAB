...
# 曝光控制损失
def L_exp_patch(I, mean_val=0.6, patch_size=16):
    # I: 增强后图像 (b×3×H×W)
    # 将图像分成不重叠 patch，计算每个 patch 的灰度平均
    gray = I.mean(dim=1, keepdim=True)            # b×1×H×W
    patches = gray.unfold(2, patch_size, patch_size).unfold(3, patch_size, patch_size)
    mean_p = patches.mean(dim=(-1,-2,-3,-4))       # b×1
    loss = ((mean_p - mean_val)**2).mean()
    return loss

# 颜色恒常性损失
def L_color_const(I):
    # I: 增强后图像 b×3×H×W
    mean_rgb = I.mean(dim=(2,3))  # b×3
    mean_gray = mean_rgb.mean(dim=1, keepdim=True)  # b×1
    loss = (mean_rgb - mean_gray).abs().mean()
    return loss
...
# 频域一致性损失
def L_freq(I, J):
    FI = torch.fft.rfft2(I, dim=(-2,-1))
    FJ = torch.fft.rfft2(J, dim=(-2,-1))
    return (FI - FJ).abs().mean()

# 边缘保持损失
def L_edge_smooth(I, J):
    # I: 光照图 b×1×H×W
    # J: 原图     b×3×H×W
    
    # 梯度
    grad_I_x = torch.abs(I[:, :, 1:, :] - I[:, :, :-1, :])  # b×1×(H-1)×W
    grad_I_y = torch.abs(I[:, :, :, 1:] - I[:, :, :, :-1])  # b×1×H×(W-1)
    
    # 权重（从 J 的梯度上计算）
    weight_x = torch.exp(-torch.mean(
        torch.abs(J[:, :, 1:, :] - J[:, :, :-1, :]),
        dim=1, keepdim=True
    ))  # b×1×(H-1)×W
    
    weight_y = torch.exp(-torch.mean(
        torch.abs(J[:, :, :, 1:] - J[:, :, :, :-1]),
        dim=1, keepdim=True
    ))  # b×1×H×(W-1)
    
    # 直接相乘，无额外切片
    loss_x = (grad_I_x * weight_x).mean()
    loss_y = (grad_I_y * weight_y).mean()
    
    return loss_x + loss_y

# 简易 InfoNCE
def L_contrast(fea1, fea2):
    b, c, h, w = fea1.shape
    # 先把特征拉成 b×N×C
    f1 = fea1.flatten(2).transpose(1, 2)  # b×N×C
    f2 = fea2.flatten(2).transpose(1, 2)  # b×N×C
    # sim: b×N×N
    sim = torch.matmul(f1, f2.transpose(1, 2))
    # 对每个 batch 单独做 cross_entropy
    N = sim.size(-1)
    labels = torch.arange(N, device=sim.device, dtype=torch.long)  # N
    loss = 0.0
    for i in range(b):
        # sim[i]: N×N, labels: (N,)
        loss += F.cross_entropy(sim[i], labels)
    return loss / b
...
def train():
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        input_edit = torch.clamp(input, 0, 1)
        ...
        l_exp_ctrl = L_exp_patch(I3, mean_val=0.6, patch_size=16) # 曝光损失
        l_color_const = L_color_const(I2) # 颜色恒畅量损失
        l_ssim = 1 - pytorch_msssim.ms_ssim(X2, im2, data_range=1.0, size_average=True, win_size=7) # 多尺度结构相似度
        loss_freq = L_freq(X2, im2) # 频率损失
        loss_edge = L_edge_smooth(I3, input_edit) # 边缘平滑损失
        L_con = L_contrast(R1, R2)  # 对比损失


        # loss5 = l_exp_ctrl * 10  + l_ssim * 10 + loss_freq * 10 + loss_edge * 10 + L_con
        # print(l_exp_ctrl)
        # print(l_color_const)
        # print(l_ssim)
        # print(loss_freq)

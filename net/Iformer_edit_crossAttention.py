...
class crossAttention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        # QKV 分支（两个输入都可以做 query）
        self.kv_x = nn.Conv2d(dim, dim * 2, kernel_size=1, bias=bias)
        self.kv_x_dwconv = nn.Conv2d(dim * 2, dim * 2, kernel_size=3, stride=1, padding=1, groups=dim * 2, bias=bias)

        self.q_y = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.q_y_dwconv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim, bias=bias)

        # 对称方向
        self.kv_y = nn.Conv2d(dim, dim * 2, kernel_size=1, bias=bias)
        self.kv_y_dwconv = nn.Conv2d(dim * 2, dim * 2, kernel_size=3, stride=1, padding=1, groups=dim * 2, bias=bias)

        self.q_x = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.q_x_dwconv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim, bias=bias)

        # 输出投影
        self.project_out = nn.Conv2d(dim * 2, dim, kernel_size=1, bias=bias)  # 拼接后输出回dim维

    def forward(self, x, y):
        b, c, h, w = x.shape

        # 分支1: Q = y, KV = x
        kv1 = self.kv_x_dwconv(self.kv_x(x))
        k1, v1 = kv1.chunk(2, dim=1)
        q1 = self.q_y_dwconv(self.q_y(y))
        ...
        # 分支2: Q = x, KV = y
        kv2 = self.kv_y_dwconv(self.kv_y(y))
        k2, v2 = kv2.chunk(2, dim=1)
        q2 = self.q_x_dwconv(self.q_x(x))

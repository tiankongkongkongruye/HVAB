...
class MSCAM(nn.Module):
    def __init__(self, in_channels):
        ...
        self.sab = SAB()
        self.mscb = MSCB(in_channels, in_channels, stride=1, kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, add=True, activation='relu6')
    
    def forward(self, input):
        temp = self.cab(input) * input
        temp = self.sab(temp) * temp
        output = self.mscb(temp)
        return output
...
if __name__ == "__main__":
    ...
    model = MSCAM(in_channels=3).cuda()
    output = model(input)
    print((output).shape)
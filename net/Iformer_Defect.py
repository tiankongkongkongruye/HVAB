...
class R_net(nn.Module):
    ...
    def forward(self, input, fea):
        ...
        #x =self.encoder(x)
        out = self.output(x)
        return torch.clamp(out + 0.1 * input, 0, 1)
...
class Highlight_Mask_Net(nn.Module):
    def __init__(self, in_channels=3, num_features=32):
        super(Highlight_Mask_Net, self).__init__()
...
class HighlightCorrectionNet(nn.Module):
    def __init__(self, in_channels=7, mid_channels=32):
        super(HighlightCorrectionNet, self).__init__()
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_channels // 2, 3, 3, 1, 1),  # RGB 输出
            nn.Sigmoid()
        )
    
    def forward(self, R, L, X):
        """
        R: 反射图 (B, 3, H, W)
        L: 光照图 (B, 1, H, W)
        X: 原图    (B, 3, H, W)
        """
        # 拼接作为特征输入
        input = torch.cat([R, L, X], dim=1)  # 3 + 1 + 3 = 7 channels
        feat = self.encoder(input)
        out = self.decoder(feat)
        
        # 与输入反射图融合（保留更多纹理）

        mask = (L > 0.9).float()  # 高光区域
        out = mask * out + (1 - mask) * R  # 仅修复高光区域
        return out
...
class net(nn.Module):
    def __init__(self):
        ...
        # self.hl_mask_net = Highlight_Mask_Net(in_channels=3, num_features=32)  # 新增高光检测模块
        # 去除反射层中的高光
        self.R_cleaner = HighlightCorrectionNet()
    ...
    def forward(self, input):
        ...
        # 新增高光 mask 检测
        # hl_mask = self.hl_mask_net(input)
        
        # 对 L 在高光区域做压缩处理
        # L_corrected = hl_mask * self.highlight_correction(L, threshold=0.8, factor=0.5) + (1 - hl_mask) * L

        R_clean = self.R_cleaner(R, L, input)

        # alpha = self.L_enhance_net(L)
        #el = torch.pow(L,alpha)
        # I = torch.pow(L,alpha) * R_clean

        L_corrected = torch.clamp(1 - L, 0, 1)  # 高光区域值低，暗区值高
        I = R_clean * (L_corrected + 1e-6)  # 让高光区域减弱，细节保留
        
        # 重构图像：在这里只做高光去除，不对低光做增强
        # I = L_corrected * R
        return L, L, R, R_clean, x, I
    
    def highlight_correction(self, img, threshold=0.8, factor=0.5):
        # 对超过阈值的像素进行压缩

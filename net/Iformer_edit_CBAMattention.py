...
import time
from net.CBAM import CBAMBlock
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type):
        ...
        # 2. 门控融合卷积：输出通道数 = dim
        self.gate_conv = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
    ...
    def forward(self, x):
        # --- 并行计算两路特征 ---
        feat      = self.norm1(x)                        # [B,C,H,W]
        attn_feat = self.attn(feat)                      # 全局自注意力
        cbam_feat = self.cbam(feat)                      # CBAM 通道+空间注意力

        # --- 生成门控并融合 ---
        gate = torch.sigmoid(self.gate_conv(feat))       # 门控系数 ∈ (0,1)
        y = gate * attn_feat + (1 - gate) * cbam_feat    # 动态加权融合

        # --- 残差连接到主干 ---
        x = x * self.skip_scale + y

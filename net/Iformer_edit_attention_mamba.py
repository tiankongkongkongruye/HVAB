...
import time
import math
from net.vmamba import VSSBlock, SS2D
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        x = x*self.skip_scale + self.attn(self.norm1(x).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
...
class F_light_prior_estimater(nn.Module):
    def __init__(
        ...
        self.depth_conv = nn.Conv2d(
            n_fea_middle, n_fea_middle, kernel_size=5, padding=2, bias=True, groups=n_fea_in)
        # self.depth_conv = Depth_conv(n_fea_middle, n_fea_middle)

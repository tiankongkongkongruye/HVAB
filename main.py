...
# from net.lformer_wave import net
...
def train():
    ...
    ### NEW ###
    # 在训练开始前实例化新的损失函数
    mi_loss_fn = MutualInformationLoss().cuda()
    kl_loss_fn = KLDivergenceLoss().cuda()
    
    # 为新损失定义权重 (这些是需要调试的超参数)
    lambda_mi = 0.1  # 互信息损失的权重
    lambda_kl = 0.01 # KL散度损失的权重
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        # 将物理损失添加到总损失中
        # loss = loss1 + loss2 + loss3 * 500 + loss4 + physical_loss
        loss = loss1 + loss2 + loss3 * 500 + loss4

        ### NEW ###
        # --- 计算新增的理论强化损失 ---
        # 1. 互信息损失 L_MI，用于确保反射 R 和光照 L 的独立性 [cite: 37, 38]
        # 我们在第一组分解结果 R1 和 L1 上计算
        mi_loss = mi_loss_fn(R1, L1)
        
        # 2. KL散度损失，作为ELBO的正则化项 
        kl_loss = kl_loss_fn(R1, L1)

        ### MODIFIED ###
        # --- 组合成最终的总损失 ---
        # 在原有损失的基础上，加上加权后的新损失项
        # loss = loss1 + loss2 + loss3 * 500 + loss4 + lambda_mi * mi_loss + lambda_kl * kl_loss + physical_loss
